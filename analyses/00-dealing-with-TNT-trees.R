...
thero_pbdb <- read_csv("data/trees_TNT/theroopod-dates-PBDB.csv")
orni_pbdb <- read_csv("data/trees_TNT/orni-dates-PBDB.csv")
...
thero_dates <- thero_pbdb %>%
  mutate(split.names = accepted_name) %>%
  separate(split.names, sep = " ", c("Genus", "species")) %>%
  select(Genus, max_ma, min_ma)

orni_dates <- orni_pbdb %>%
  mutate(split.names = accepted_name) %>%
  separate(split.names, sep = " ", c("Genus", "species")) %>%
  select(Genus, max_ma, min_ma)
